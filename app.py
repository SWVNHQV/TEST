...
/* Dedicated opaque surface for Data Quality finding explanations. */
.st-key-dq-explain-result {{
    background:#ffffff !important;
    background-color:#ffffff !important;
...
.st-key-dq-explain-result [data-testid="stMarkdownContainer"],
.st-key-dq-explain-result p,
.st-key-dq-explain-result li,
.st-key-dq-explain-result h1,
.st-key-dq-explain-result h2,
.st-key-dq-explain-result h3,
.st-key-dq-explain-result h4,
.st-key-dq-explain-result strong,
.st-key-dq-explain-result td,
.st-key-dq-explain-result th {{
    color:#243b55 !important;
    opacity:1 !important;
}}
.st-key-dq-explain-result [data-testid="stMarkdownContainer"] table {{
    background:#ffffff !important;
}}
    ...
    if dq.empty:
        ...
        if st.button("Explain selected finding", type="primary", key="dq_explain"):
            ...
            with st.spinner("Copilot is checking the exact finding and connected records..."):
                try:
                    answer = explain_finding(context)
                    with st.container(key="dq-explain-result"):
                        st.markdown("<div class='copilot-result-label'>AI COPILOT ANALYSIS</div>", unsafe_allow_html=True)
                        st.markdown(answer)
                except Exception as exc:
...
# -----------------------------------------------------------------------------
    ...
    sheet = st.selectbox("Sheet", visifrom __future__ import annotations
